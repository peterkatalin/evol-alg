...
def get_parameters():
    ...
    gen = 50                                                            #generaciok szama
    select = int(n_pop / 2)                                             #hany elemet valasztunk ki a generaciobol (Szelekcional)
...
def genetikus_algoritmus(chroms ,gen, taska_meret, ertekek, sulyok, n_pop,select,p_cross,p_mut):
    ...
    for i in range(gen):
        ...
        keresztezodes_n(chroms,select,p_cross,n_pop,parent_probability,10)
        ...
        print(legjobb_egyed)
        best.append(legjobb_egyed.getValue()/100)
